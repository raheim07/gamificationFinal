    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <motion.div custom={0} variants={card} initial="initial" animate="animate">
        <div className="flex items-center gap-2 mb-1">
          ...
          >
            <Flame className="h-5 w-5" style={{ color: streak > 0 ? "#F97316" : "var(--text-dim)" }} />
          </motion.div>
        </div>
        <p className="text-sm italic" style={{ color: "var(--text-dim)" }}>{motivationalText}</p>
      </motion.div>

      {/* Step Input */}
      <motion.div custom={1} variants={card} initial="initial" animate="animate" className="glass-card rounded-2xl p-4 sm:p-5 ambient-glow relative">
      ...
      {/* Animated Stats Row */}
      <motion.div custom={2} variants={card} initial="initial" animate="animate" className="grid grid-cols-3 gap-2 sm:gap-3">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            <span className="text-xs uppercase tracking-wider" style={{ color: "var(--text-dim)" }}>Current Level</span>
            <div className="flex items-center gap-2 mt-1">
              <Medal className="h-5 w-5" style={{ color: "#0EA5A4" }} />
              <span className="font-[var(--font-montserrat)] text-lg font-bold" style={{ color: "var(--text-primary)" }}>{level.name}</span>
            </div>
            <div className="mt-2 w-32 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--ring-track)" }}>
              <motion.div
      ...
      >
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="text-sm font-bold" style={{ color: "var(--text-primary)" }}>Weekly Challenge</h3>
            <p className="text-xs" style={{ color: "var(--text-dim)" }}>Reach 35,000 steps this week</p>
          </div>
        </div>
      ...
      {/* Badges */}
      <motion.div custom={6} variants={card} initial="initial" animate="animate" className="glass-card rounded-2xl p-4 sm:p-5">
        <h3 className="text-xs font-medium mb-3 flex items-center gap-2" style={{ color: "var(--text-secondary)" }}>
      ...
      {/* Leaderboard */}
      <motion.div custom={7} variants={card} initial="initial" animate="animate" className="glass-card rounded-2xl p-4 sm:p-5">
        <h3 className="text-xs font-medium mb-3 flex items-center gap-2" style={{ color: "var(--text-secondary)" }}>
          <Trophy className="h-4 w-4" style={{ color: "#0EA5A4" }} />
          Leaderboard
        </h3>
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      ...
                      style={{
                        color: isFirst ? "#FACC15" : "var(--text-dim)",
                      }}
                    >
                      {idx + 1}
                    </span>
      ...
      {/* Support Messages */}
      {messages.length > 0 && (
        <motion.div custom={8} variants={card} initial="initial" animate="animate" className="glass-card rounded-2xl p-4 sm:p-5">
          <h3 className="text-xs font-medium mb-3 flex items-center gap-2" style={{ color: "var(--text-secondary)" }}>
          ...
          <div className="space-y-2 max-h-40 overflow-y-auto">
            {messages.map((msg) => (
              ...
              >
                <p className="text-[11px] mb-0.5" style={{ color: "var(--text-dim)" }}>From {msg.from}</p>
